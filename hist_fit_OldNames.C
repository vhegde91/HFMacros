...
int nietaHF=26,niphiHF=36,minEntryPerBin=10;
vector<double> E1,E2,dE1,dE2;
int fitlinecolor=kBlue;
void hist_fit(const char* ipfname){
  ...
  for(int i=0;i<nE1E2Cuts;i++){
	...
	ieta=-(j+16);
      ...
      sprintf(name,"E2E1_ieta%i",ieta);
      h2=(TH2D*)dir->FindObjectAny(name);
      Hlist2.Add(h2);

      sprintf(name,"E2E1Prof_ieta%i",ieta);
	...
	c_ieta->Print(name_pdf,title);
      ...
      sprintf(name,"P_RatiovsElong_ieta%i",ieta);
