...
void my_hadd() {
  ...
  /*------------  if(gSystem->AccessPathName("hsimple1.root")) {
   ...
   //========================Add list of files to be combined here=================================
   FileList->Add( TFile::Open("170to300_MC_E2E1Hists.root") );
   crossSection.push_back(1);
   FileList->Add( TFile::Open("300to470_MC_E2E1Hists.root") );
   crossSection.push_back(0);
   //========================Add list of files to be combined here==================================
...
void MergeRootfile( TDirectory *target, TList *sourcelist ) {
   ...
   while ( (key = (TKey*)nextkey())) {
	 ...
	 Double_t my_n=(h1->GetEntries());
	 if(my_n!=0){
	   h1->Scale(my_Luminosity*crossSection[0]);
	 }
	 int my_i=1;
         ...
         while ( nextsource ) {
	       ...
	       Double_t my_n2=(h2->GetEntries());
	       if(my_n2!=0){
		 h2->Scale(my_Luminosity*crossSection[my_i]);
	       }
	       h1->Add( h2 );
	       my_i++;
	       delete h2;
